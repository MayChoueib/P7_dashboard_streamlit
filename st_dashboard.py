...
def box_plot_all(df, num_client, cols, decision):
    ...
    sns.swarmplot(data=df_cust, x='var', y='val', linewidth=1, color=df_cust['color'].unique()[0],
                  marker='p', size=8, ec='k', label='client', ax=ax)
...
#Titre de la page
st.set_page_config(page_title="Prêt à dépenser - Demandes de prêts", layout="wide")

st.title("Prêt à dépenser - Demandes de prêts")
st.write("Dashboard réalisée par May CHOUEIB - Parcours Data Science - projet 7")
...
if num != '<Select>':
    ...
    if influence:
       ...
       explant = Explanation(np.reshape(np.array(shap_val, dtype = 'float'),(1,-1)),
                             base_value,
       ...
       with shp1:
           ...
           fig = waterfall(explant[0], max_display=nb_features, show=False)
           st.pyplot(fig)
           exp = st.expander("Explanation of the SHAP waterfall plot?")
           exp.write("The bottom of the above waterfall plot starts as the expected value of the model output \
